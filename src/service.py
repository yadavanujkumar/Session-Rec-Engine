...
class RecommendationService:
    ...
    def _get_model_recommendations(self, session_id: str) -> List[str]:
        ...
        """
        # Get item sequence
        item_ids = self.session_store.get_item_sequence(session_id, self.sequence_length)
        ...
        # Convert to indices
        item_indices = [self.item_catalog.get_item_index(item_id) for item_id in item_ids]
        
        # Pad sequence if needed
        if len(item_indices) < self.sequence_length:
            item_indices = [0] * (self.sequence_length - len(item_indices)) + item_indices
        else:
        ...
        # Get predictions
        with torch.no_grad():
            top_items, top_scores = self.model.predict_next_items(item_seq, top_k=self.top_k)
        
        # Convert indices back to item IDs
        top_items_np = top_items.cpu().numpy()[0]
        recommendations = [self.item_catalog.get_item_id(int(idx)) for idx in top_items_np]
        
        # Filter out unknown items
        recommendations = [item_id for item_id in recommendations if item_id != "unknown"]
